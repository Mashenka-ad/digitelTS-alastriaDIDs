...
const keyDataEntityProvider = fs.readFileSync(
  '../keystores/providerData.json'
)
const keystoreDataEntityProvider = JSON.parse(keyDataEntityProvider)
...
const entityProviderKeystore = keystoreDataEntityProvider

let entityProviderPrivateKey
try {
  entityProviderPrivateKey = keythereum.recover(
    configData.addressProviderPassword,
    entityProviderKeystore
  )
} catch (error) {
  console.log('ERROR: ', error)
  process.exit(1)
...
const entityProviderIdentity = new UserIdentity(
  web3,
  `0x${entityProviderKeystore.address}`,
  entityProviderPrivateKey
)
// End data
...
function preparedAlastriaId() {
  const preparedId = transactionFactory.identityManager.prepareAlastriaID(
    web3,
    entityProviderKeystore.address
  )
  return preparedId
}
...
function createAlastriaId() {
  const txCreateAlastriaID = transactionFactory.identityManager.createAlastriaIdentity(
    web3,
    configData.entityProviderPubk.substr(2)
  )
        ...
        .on('receipt', function (receipt) {
          ...
          web3.eth
            ...
            .then((AlastriaIdentity) => {
              ...
              configData.didEntityProvider = alastriaDID
              fs.writeFileSync(
                '../configuration.json',
                JSON.stringify(configData)
              )
